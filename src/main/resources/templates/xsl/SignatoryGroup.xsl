<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    <xsl:output method="xml"/>
    <xsl:template match="/">
        <page xmlns:comm="http://ws.online.fnb.co.za/v1/common/"
              id="WrapupLandingPage"
              title="Switch" template="main" layout="" version="1">
            <symbol xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                    xsi:type="comm:formLayout">
                <comm:form comm:action=""
                           comm:name="WrapupLandingForm">
                    <comm:sections comm:width="full">
                        <comm:symbol xsi:type="comm:boxContainer"
                                     comm:id="boxDiv">
                            <comm:box xsi:type="comm:box">
                                <comm:boxSymbol xsi:type="comm:textHeading"
                                                comm:size="4">
                                    <comm:value>Added Signatories</comm:value>
                                </comm:boxSymbol>
                                <comm:boxSymbol xsi:type="comm:input"
                                                comm:name="fullName"
                                                comm:label="Full Name *"
                                                comm:inputType="text" comm:message=""
                                                comm:unCheckedValue="No"
                                                comm:selected="true"
                                                comm:required="true"
                                                comm:readonly="true"
                                                comm:maxlength="50">
                                    <comm:value>
                                        <xsl:value-of
                                                select="signatoryModel/fullName"/>
                                    </comm:value>
                                </comm:boxSymbol>
                                <comm:boxSymbol xsi:type="comm:input"
                                                comm:name="capacity"
                                                comm:label="Capacity *"
                                                comm:inputType="text" comm:message=""
                                                comm:unCheckedValue="No"
                                                comm:selected="true"
                                                comm:errorMessage="{signatoryModel/signatoryErrorModel/capacity}"
                                                comm:required="true"
                                                comm:maxlength="50">
                                    <comm:value>
                                        <xsl:value-of
                                                select="signatoryModel/capacity"/>
                                    </comm:value>
                                </comm:boxSymbol>
                                <comm:boxSymbol xsi:type="comm:input"
                                                comm:name="idNumber"
                                                comm:label="ID Number *"
                                                comm:inputType="text" comm:message=""
                                                comm:unCheckedValue="No"
                                                comm:selected="true"
                                                comm:required="true"
                                                comm:maxlength="50">
                                    <comm:value>
                                        <xsl:value-of
                                                select="signatoryModel/idNumber"/>
                                    </comm:value>
                                </comm:boxSymbol>
                                <comm:boxSymbol xsi:type="comm:input"
                                                comm:name="Group"
                                                comm:label="Group *"
                                                comm:inputType="text" comm:message=""
                                                comm:unCheckedValue="No"
                                                comm:selected="true"
                                                comm:errorMessage="{signatoryModel/signatoryErrorModel/group}"
                                                comm:required="true"
                                                comm:maxlength="50">
                                    <comm:value>
                                        <xsl:value-of
                                                select="signatoryModel/group"/>
                                    </comm:value>
                                </comm:boxSymbol>
                                <comm:boxSymbol xsi:type="comm:input"
                                                comm:name="confirmationCheckMandateResolutionSigma"
                                                comm:inputType="checkbox"
                                                comm:errorMessage="{signatoryModel/signatoryErrorModel/checkDocConfirm}"
                                                comm:unCheckedValue="No"
                                                comm:selected="false">
                                    <comm:value/>
                                    <comm:inputItem
                                            comm:id="confirm"
                                            comm:label="I confirm that the documents have been uploaded to sigma"
                                            comm:type="checkbox"
                                            comm:value="true"
                                            comm:unCheckedValue="false"
                                            comm:selected="{signatoryModel/checkDocConfirm}"/>
                                </comm:boxSymbol>
                            </comm:box>
                        </comm:symbol>
                    </comm:sections>
                </comm:form>
            </symbol>
            <symbol xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                    xsi:type="comm:footer" comm:text="" comm:textAlign="left"
                    comm:buttonAlign="right">
                <comm:baseButton comm:id="cancel"
                                 comm:url="app-domain/mandates-and-resolutions/backSignature"
                                 comm:target="main" comm:formSubmit="false" comm:tooltip=""
                                 comm:label="Back"/>
                <comm:baseButton comm:id="next"
                                 comm:url="app-domain/mandates-and-resolutions/updateAddedSignatory/{signatoryModel/userInList}/{signatoryModel/userInAccount}"
                                 comm:target="main" comm:formSubmit="true" comm:tooltip=""
                                 comm:label="Update"/>
            </symbol>
        </page>
    </xsl:template>
</xsl:stylesheet>